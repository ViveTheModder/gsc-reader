...
public class GSC 
{
	...
	private String game;
	private static final int GSAC=0x47534143, GSCD=0x47534344, GSCF=0x47534346, GSHD=0x47534844, GSDT=0x47534454, SCENE_MINUS_1 = 0xFFFFFFFF;
	...
	private String readParams(byte[] gsdt, String[] info, short currFuncId, int numParams, boolean isFunc) throws IOException
	{
		...
		for (int i=0; i<numParams; i++)
		{
			...
			if (paramDataType==10) 
			{
				...
				//replace the 80 in condition IDs (or indices) applied to opponents with zero to prevent out of range index
	...
	private String[] getInfoFromCall(short callId, short functionIdForProp, boolean isFunc, boolean isParam) throws IOException
	{
		...
		while (sc.hasNextLine())
		{
			...
			if (callIdFromLine==callId) 
			{
				...
				{
					/* a whole 'nother parameter had to be added to this method because of how
					evil Spike devs were for adding properties with different purposes that
					end up sharing the same ID (hence why the current function ID is needed) */
					if (functionIdForProp==Short.parseShort(lineAsArray[1]))
						info = lineAsArray;
				}
			}
		}
	...
	public String getGscErrors() throws IOException
	{
		...
		//file size checks
		if (gscSize>24576) gscError+="* GSC exceeds file size limit of 24 KiB!\n";
		gsc.seek(8);
		short sizeFromGscf = LittleEndian.getShort(gsc.readShort());
		if (sizeFromGscf+32 != gscSize) 
			gscError+="* GSC file size ("+gscSize+") does NOT match file size ("+(sizeFromGscf+32)+") specified in header!\n";
		
		for (int i=0; i<gscHeaderInfo.length; i++)
		{
			gsc.seek(gscHeaderAddr[i]);
			int input = gsc.readInt();
			if (input!=gscHeaderInfo[i]) gscError+="* GSC does not contain "+gscHeaderInfoStr[i]+" in header!\n";
		}
		gsc.seek(72);
		...
		if (gsc.readInt()!=GSDT) gscError+="* GSC does NOT contain GSDT indicator!\n";
		return gscError;
	}
